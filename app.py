...
postgresql = os.environ.get('POSTGRES',False)
if postgresql:
    user = os.environ.get('POSTGRES_USER')
    password = os.environ.get('POSTGRES_PASSWORD')
    print(f"postgresql://{user}:{password}@postgres_db:5432/test")
    app.config['SQLALCHEMY_DATABASE_URI'] = f"postgresql://{user}:{password}@postgres_db:5432/test"
else:
